...
function InquiryModal({ open, onClose, onSubmit }) {
  ...
  }, [firstName, lastName, email, phone, message, open, validate]);
  if (!open) return null;
  function autoGrow() {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="inqTitle"
      aria-describedby="inqDesc"
    >
      <div
        className="inqModal__backdrop"
        onClick={!submitting ? resetAndClose : undefined}
      />
      <div
        className={`inqShell${submitted ? " inqShell--done" : ""}`}
        ref={panelRef}
        data-phase={submitted ? "done" : "form"}
      >
        {!submitted && (
          <form className="inqLayout" onSubmit={submit} noValidate>
            ...
            <div className="inqGrid">
              <label className="inqField">
                <span className="inqField__label">First Name</span>
                <input
                  ...
                  aria-invalid={
                    attempted && errors.firstName ? true : undefined
                  }
                />
                {attempted && errors.firstName && (
                  <span className="inqErr">{errors.firstName}</span>
                )}
              </label>
              <label className="inqField">
                <span className="inqField__label">Last Name</span>
                <input
                  data-field="lastName"
                  className="inqInput"
                  value={lastName}
                  onChange={handleChange(setLastName)}
                  aria-invalid={attempted && errors.lastName ? true : undefined}
                />
                {attempted && errors.lastName && (
                  <span className="inqErr">{errors.lastName}</span>
                )}
              </label>
              <label className="inqField">
                <span className="inqField__label">Email</span>
                <input
                  className="inqInput"
                  type="email"
                  value={email}
                ...
                {attempted && errors.email && (
                  <span className="inqErr">{errors.email}</span>
                )}
              </label>
              <label className="inqField">
                <span className="inqField__label">Phone</span>
                <input
                  className="inqInput"
                  type="tel"
                  value={phone}
                ...
                {attempted && errors.phone && (
                  <span className="inqErr">{errors.phone}</span>
                )}
              </label>
              <label className="inqField inqField--full">
                <span className="inqField__label">Message</span>
                <textarea
                  ref={messageRef}
                  ...
                  value={message}
                  onChange={(e) => {
                    setMessage(e.target.value.slice(0, maxMessage));
                    autoGrow();
                    if (attempted) setErrors(validate());
                  }}
                  aria-invalid={attempted && errors.message ? true : undefined}
                  placeholder="Describe your docking needs, vessel size, timeline..."
                />
                {attempted && errors.message && (
                  <span className="inqErr">{errors.message}</span>
                )}
                <span className="inqCounter" aria-live="polite">
            ...
            <div className="inqFooter inqFooter--actions">
              <div className="inqActions inqActions--right">
                ...
                <button
                  ...
                  className="inqBtn inqBtn--primary"
            ...
            <div className="inqLive" aria-live="assertive" aria-atomic="true">
        ...
        {submitted && (
          <div className="inqResult">
            ...
            <p className="inqResult__text">
              We will reach out within 24 hours. You can also call{" "}
              {PHONE_NUMBER} any time for immediate assistance.
            </p>
            <div className="inqResult__actions">
              <button
                className="inqBtn inqBtn--primary"
                onClick={resetAndClose}
              >
